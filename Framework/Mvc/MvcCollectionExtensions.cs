...
namespace JJ.Framework.Mvc
{
    ...
    {
        private static readonly object _qualifierDictionaryLock = new object();
        private static readonly Dictionary<int, Qualifier> _qualifierDictionary = new Dictionary<int, Qualifier>();
        ...
        public static IDisposable BeginItem(this HtmlHelper htmlHelper, Expression<Func<object>> expression)
        {
            ...
            string identifier = ExpressionHelper.GetName(expression);
        ...
        private class Qualifier : IDisposable
        {
            ...
            public void IncrementIndex(HtmlHelper htmlHelper)
            {
                ...
                if (!(_nodes.Peek() is CollectionNode node))
                {
            ...
            private void WriteHiddenIndexField(HtmlHelper htmlHelper, int index)
            {
                ...
                string indexFieldName = $"{collectionName}.index";
                ...
                string html =
                    $@"<input type=""hidden"" name=""{htmlHelper.Encode(indexFieldName)}"" value=""{
                            htmlHelper.Encode(index)
                        }"" autocomplete=""off""/>";
