...
namespace JJ.Framework.Soap.Tests.Server
{
    public class TestService : ITestService
    {
        ...
        public string GetStringValue() => "Hello world!";

        public string SendAndGetStringValue(string stringValue) => $"{stringValue} {"to you too!"}";
        ...
        public ComplicatedType SendAndGetComplicatedObject(ComplicatedType complicatedObject)
        {
            ...
            checked
            {
                var complicatedObject2 = new ComplicatedType
                {
                    ...
                    Int16 = (short)(complicatedObject.Int16 + 1),
                    UInt16 = (ushort)(complicatedObject.UInt16 + 1),
                    ...
                    // Collections
                    Array = complicatedObject.Array.Concat(new[] { 2, 2 }).ToArray(),
                    ListOfT = complicatedObject.ListOfT.Concat(new List<int> { 2, 2 }).ToList(),
                    IListOfT = complicatedObject.IListOfT.Concat(new[] { 2, 2 }).ToList(),
                    ICollectionOfT = complicatedObject.ICollectionOfT.Concat(new[] { 2, 2 }).ToArray(),
                    IEnumerableOfT = complicatedObject.IEnumerableOfT.Concat(new[] { 2, 2 }).ToArray(),
                    ...
                    ListOfCompositeObjects = complicatedObject.ListOfCompositeObjects.Concat(
                                                                  new[]
                                                                  {
                    ...
                    ArrayOfBoolean = complicatedObject.ArrayOfBoolean.Concat(new[] { true, false }).ToArray(),
                    //ArrayOfByte = Enumerable.Concat(complicatedObject.ArrayOfByte, new byte[] { 0x10, 0x20 }).ToArray(),
                    ArrayOfSByte = complicatedObject.ArrayOfSByte.Concat(new sbyte[] { -2, 2 }).ToArray(),
                    ArrayOfInt16 = complicatedObject.ArrayOfInt16.Concat(new short[] { -2, 2 }).ToArray(),
                    ArrayOfUInt16 = complicatedObject.ArrayOfUInt16.Concat(new ushort[] { 1, 2 }).ToArray(),
                    ArrayOfInt32 = complicatedObject.ArrayOfInt32.Concat(new[] { -2, 2 }).ToArray(),
                    ArrayOfUInt32 = complicatedObject.ArrayOfUInt32.Concat(new uint[] { 1, 2 }).ToArray(),
                    ArrayOfInt64 = complicatedObject.ArrayOfInt64.Concat(new long[] { -2, 2 }).ToArray(),
                    ArrayOfUInt64 = complicatedObject.ArrayOfUInt64.Concat(new ulong[] { 1, 2 }).ToArray(),
                    ArrayOfChar = complicatedObject.ArrayOfChar.Concat(new[] { 'a', 'b' }).ToArray(),
                    ArrayOfDouble = complicatedObject.ArrayOfDouble.Concat(new[] { 1.23E4, 2.34E5 }).ToArray(),
                    ArrayOfSingle = complicatedObject.ArrayOfSingle.Concat(new[] { 1.23E4f, 2.34E5f }).ToArray(),
                    ArrayOfString = complicatedObject.ArrayOfString.Concat(new[] { "Hello", "world", "!" }).ToArray(),
                    ArrayOfGuid = complicatedObject.ArrayOfGuid.Concat(new[] { Guid.NewGuid(), Guid.NewGuid() }).ToArray(),
                    ArrayOfTimeSpan = complicatedObject.ArrayOfTimeSpan.Concat(new[] { TimeSpan.Parse("01:00"), TimeSpan.Parse("02:00") })
                                                       .ToArray(),
                    ArrayOfDateTime = complicatedObject.ArrayOfDateTime
                                                       .Concat(new[] { DateTime.Parse("2001-02-03 04:05"), DateTime.Parse("2006-07-08 09:10") })
                                                       .ToArray(),
                    ArrayOfEnumType = complicatedObject.ArrayOfEnumType.Concat(new[] { EnumType.EnumMember1, EnumType.EnumMember2 }).ToArray()
