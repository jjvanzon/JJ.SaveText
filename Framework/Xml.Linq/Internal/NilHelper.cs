...
namespace JJ.Framework.Xml.Linq.Internal
{
    internal static class NilHelper
    {
        ...
        {
            var destElement = new XElement(destXName, null);
            var nilAttribute = new XAttribute(GetNilXNamespace() + NIL_ATTRIBUTE_NAME, true);
            destElement.Add(nilAttribute);
            return destElement;
        }

        public static bool HasNilAttribute(XElement element) => element.Attributes(GetNilXNamespace() + NIL_ATTRIBUTE_NAME).Any();
    }
}